...
use IanRodrigues\FalAi\Image\Concerns\FetchesResults;
use IanRodrigues\FalAi\Image\Concerns\MapsAttachments;
use Illuminate\Contracts\Events\Dispatcher;
use Illuminate\Http\Client\PendingRequest;
use Illuminate\Support\Facades\Http;
use InvalidArgumentException;
use Laravel\Ai\Contracts\Gateway\ImageGateway as ImageGatewayContract;
use Laravel\Ai\Contracts\Providers\ImageProvider;
use Laravel\Ai\Responses\ImageResponse;
use RuntimeException;

class Gateway implements ImageGatewayContract
{
    use FetchesResults;
    use MapsAttachments;
    ...
    public function generateImage(
        ...
        if (! $provider instanceof FalProvider) {
            throw new InvalidArgumentException(
                'fal image gateway requires a FalProvider; got ['.$provider::class.'].',
            );
        }
